...
def add_figure_templates(default=None):
    ...
    def make_template(name):
        ...
        template.data.scatter = (
            go.Scatter(marker_line_color=theme_config["plot_bgcolor"]),
        )
        template.data.scattergl = (
            go.Scattergl(marker_line_color=theme_config["plot_bgcolor"]),
        )
    ...
    if default in ["mantine_light", "mantine_dark"]:
        ...
        raise ValueError(
            f"unrecognized {default=}, allowed values are 'mantine_light' and 'mantine_dark'"
        )
